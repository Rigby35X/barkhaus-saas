 ...
 * Handles image uploads for animal photos using Cloudinary
 */
...
export async function POST({ request }) {
    try {
        ...
        const section = formData.get('section') || 'animals';
        const orgId = formData.get('orgId') || '3';
        ...
        const publicId = `happy-paws/${section}/${orgId}/${timestamp}-${randomId}`;

        // Try to upload to Cloudinary if configured, otherwise use temporary solution
        const cloudinaryCloudName = import.meta.env.CLOUDINARY_CLOUD_NAME;
        const cloudinaryUploadPreset = import.meta.env.CLOUDINARY_UPLOAD_PRESET;
        ...
        if (cloudinaryCloudName && cloudinaryUploadPreset) {
            try {
                ...
                cloudinaryFormData.append('folder', `happy-paws/${section}`);
